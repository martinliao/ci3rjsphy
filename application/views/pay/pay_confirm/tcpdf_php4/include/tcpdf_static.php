	...
	* @since 5.9.121 (2011-09-28)
	...
	public static function sendOutputData($data, $length) {
		if (!isset($_SERVER['HTTP_ACCEPT_ENCODING']) OR empty($_SERVER['HTTP_ACCEPT_ENCODING'])) {
			...
			header('Content-Length: '.urlencode($length));
		}
		echo $data;
	}
	...
	public static function getRandomSeed($seed='') {
		$rnd = uniqid(crypto_rand(99999,999999).microtime(true), true);
		...
		return $rnd.$seed.__FILE__.time().microtime(true);
	}
	...
	public static function fileGetContents($file) {
		...
		if (preg_match('%^(https?)://%', $url)
			...
			if (empty($urldata['query'])) {
				$host = $protocol.'://dcsdcourse.taipei.gov.tw';
		...
		if (isset($_SERVER['SCRIPT_URI'])
		    ...
		    $urldata = @parse_url("https://dcsdcourse.taipei.gov.tw");
